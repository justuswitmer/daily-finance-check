 ...
 */
import { getTwilioClient } from "./conversation.js";
import { config } from "./config.js";

async function main() {
  if (config.recipients.length < 2) {
    throw new Error(
      "Group MMS needs at least 2 recipients (+ your Twilio number = 3 participants).",
    );
  }

  const convo = await getTwilioClient().conversations.v1.conversations.create({
